...
module wf_mpi
  ...
  real, parameter :: qmin = 1.0E-06
  real, parameter ::   ks = 0.00 ! roughness
...
contains
  ...
  do i=i_mystart+1,i_myend-1
     ...
     if (qpar > qmin) then
        ...
        !else
          !qout(4,i,j)=kapa*yplus*rei
           q(5,l1)=kapa*yplus*rei
        !end if
     else
        q(2:4,l1)=zero
        q(5,l1)=0.1/re0
...
SUBROUTINE fdjac1(n,x,fjac)
  ...
  real,parameter ::kapa=0.41,b=5.45
  if(re0*yp0*x(1)>11.3)then
     fjac(1,1)=log(re0*yp0*x(1))/kapa+b+1./kapa
  else
...
!  if(x(1).lt.1.e-4) x(1)=1.e-4
  ...
  if(ksp>=2.25)then
  if(ksp.le.90)then
     deltb=(b-8.5+1/kapa*log(ksp))*sin(0.4258*(log(ksp)-0.811))
  else
     deltb=b-8.5+1/kapa*log(ksp)
  end if
  end if
  if(re0*yp0*x(1).gt.11.3)then
     fvec(1)=(1.*log(re0*yp0*x(1))/kapa+b-deltb)*x(1)-u0
  else
