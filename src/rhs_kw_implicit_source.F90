...
subroutine rhs_kw_implicit_source ()
  ...
  do i = ista, iend
     ...
     dk(i,j,k) = one + Bs(i,j,k)* dtev(i,j,k) * q(6,i,j,k)
     dw(i,j,k) = one + cw2      * dtev(i,j,k) * q(6,i,j,k)

     rh(1,i,j,k) = - dtev(i,j,k) * aj(i,j,k) * rh(1,i,j,k)
     rh(2,i,j,k) = - dtev(i,j,k) * aj(i,j,k) * rh(2,i,j,k)

  end do
  end do
  end do

end subroutine rhs_kw_implicit_source
